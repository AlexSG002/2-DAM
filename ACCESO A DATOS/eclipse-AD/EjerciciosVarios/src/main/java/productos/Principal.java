...
import crearDepartamentosXML.departamentos;
import xmlproductos.Producto;
...
//				"----------");
		...
		for (;;) { // recorro el fichero
			...
			if (codigoPro != 0) {
				...
				pvp = file.readDouble();
				xmlproductos.Producto pro = new xmlproductos.Producto(codigoPro, nombrePro.trim(), existencias, pvp, 0, 0, "SIN ESTADO");
				lista.add(pro);
			...
			if (codigoPro < 1 || codigoPro > 99) {
				...
				if (posicionProd >= fiprod.length()) {
					...
					if (codPro == codigoPro) {
						...
						System.out.println("Localizado producto: " + codigoPro + ", se actualiza, existencias actuales: "+exis);
					} else {
